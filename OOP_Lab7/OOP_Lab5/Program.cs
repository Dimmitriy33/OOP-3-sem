...
namespace OOP_Lab6
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                if (TransportEl.SailboatsCount > 2)
                    throw new SailboatsOverflow(sailboat);
            }
            catch(SailboatsOverflow ex)
            {
                Console.WriteLine(ex.Message);
            ...
            try
            {
                new OutOfRangeExceptions(boat.captain.age, boat);
                new OutOfRangeExceptions(sailboat.captain.age, sailboat);
                new OutOfRangeExceptions(ship.captain.age, ship);
                new OutOfRangeExceptions(corvette.captain.age, corvette);
                new OutOfRangeExceptions(streamer.captain.age, streamer);
            }
            catch
            {
            ...
            try
            {
                void Test(int num) 
                {
                    ...
                    {
                        flag = 1;
                        throw new OOP_Lab7.MyExceptions.ArgumentException();
                    }
                }
                Test(3);
            }
            catch(OOP_Lab7.MyExceptions.ArgumentException ex)
            {
                Console.WriteLine(ex.Message);
            }
            finally
            ...
            int MyCourse = 2;
            Debug.Assert(MyCourse == 3, "LOL, u on the second course, boy");
        }
    }
}
